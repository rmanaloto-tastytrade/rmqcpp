...
vcpkg_cmake_configure(
    ...
    OPTIONS 
        -DBDE_BUILD_TARGET_CPP17=26
        -DBDE_BUILD_TARGET_CPP26=ON
        -DCMAKE_CXX_STANDARD=26
        -DCMAKE_CXX_STANDARD_REQUIRED=ON
        -DCMAKE_CXX_EXTENSIONS=OFF
        -DCMAKE_C_COMPILER=/opt/homebrew/opt/llvm/bin/clang
        -DCMAKE_CXX_COMPILER=/opt/homebrew/opt/llvm/bin/clang++
        "-DCMAKE_CXX_FLAGS=-include cstdlib -include new"
        -DBBS_BUILD_SYSTEM=1
...
# Strip linux-only libs from pkgconfig on Apple (librt, libstdc++) after fixup.
if(APPLE)
    file(GLOB _pcfiles
        "${CURRENT_PACKAGES_DIR}/lib/pkgconfig/*.pc"
        "${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/*.pc")
    foreach(_pc ${_pcfiles})
        vcpkg_replace_string("${_pc}" " -lrt" "")
        vcpkg_replace_string("${_pc}" "-lrt " "")
        vcpkg_replace_string("${_pc}" " -lstdc++" "")
        vcpkg_replace_string("${_pc}" "-lstdc++ " "")
    endforeach()

    # The generated bslTargets.cmake injects -lrt/-lstdc++ into INTERFACE_LINK_LIBRARIES.
    # Those do not exist on Apple, so strip them to avoid link errors.
    set(_bsl_targets "${CURRENT_PACKAGES_DIR}/share/bsl/bslTargets.cmake")
    if(EXISTS "${_bsl_targets}")
        vcpkg_replace_string("${_bsl_targets}" ";-lrt;-lstdc++" "")
        vcpkg_replace_string("${_bsl_targets}" "-lrt;" "")
        vcpkg_replace_string("${_bsl_targets}" "-lstdc++;" "")
        vcpkg_replace_string("${_bsl_targets}" " -lrt" "")
        vcpkg_replace_string("${_bsl_targets}" " -lstdc++" "")
    endif()
endif()
